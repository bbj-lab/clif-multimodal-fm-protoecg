...
def _simple_events(
    ...
    cols = [
        ...
        (pl.col(val_col).cast(pl.Float64) if val_col else pl.lit(None).cast(pl.Float64)).alias("value"),
        (pl.col(val_cat_col).cast(pl.Utf8) if val_cat_col else pl.lit(None).cast(pl.Utf8)).alias("value_cat"),
    ]
    return df.select(cols)


def _filter_included(df: pl.DataFrame, code_prefix: str, cat_col: str, profile: CodeProfile) -> pl.DataFrame:
    """Filter DataFrame to only included codes."""
    if not profile.included_codes:
        return df
    included_cats = {
        c[len(code_prefix):] for c in profile.included_codes
...
def build_all_base_events(
    ...
    for hid in all_hosp_ids:
        ...
        if age is not None:
            demo_rows.append((hid, admit, f"DEMO//AGE_{age_bucket(int(age))}", None, None))
    ...
    if len(diags) > 0 and "poa_present" in diags.columns:
        poa = diags.filter(pl.col("poa_present") == 1)
        if len(poa) > 0:
            # Need admit times per hospitalization
            admit_df = pl.DataFrame({
                "hospitalization_id": list(hosp_lookup.keys()),
                "_admit": [h["admission_dttm"] for h in hosp_lookup.values()],
            })
            poa_events = (
                poa.join(admit_df, on="hospitalization_id", how="left")
                .select(
                    pl.col("hospitalization_id"),
                    pl.col("_admit").alias("time"),
                    (pl.lit("ICD_POA//") + pl.col("diagnosis_code").cast(pl.Utf8)).alias("code"),
                    pl.lit(None).cast(pl.Float64).alias("value"),
                    pl.lit(None).cast(pl.Utf8).alias("value_cat"),
                )
            )
            frames.append(poa_events)

    # ── 3. ADT ──────────────────────────────────────────────────────
    adt = loader.load_for_hospitalizations("adt", all_hosp_ids)
    if len(adt) > 0:
        frames.append(_simple_events(
            adt, "in_dttm",
    ...
    if len(labs) > 0:
        ...
        if len(lb) > 0:
            ...
            if len(lb_orders) > 0:
                frames.append(lb_orders.select(
                    pl.col("hospitalization_id"),
                    pl.coalesce("lab_order_dttm", "lab_collect_dttm").alias("time"),
                    (pl.lit("LAB_ORDER//") + pl.col("lab_category").cast(pl.Utf8)).alias("code"),
                    pl.lit(None).cast(pl.Float64).alias("value"),
                    pl.lit(None).cast(pl.Utf8).alias("value_cat"),
                ))
            ...
            if len(lb_results) > 0:
                # Prefer lab_result_dttm, fallback to lab_collect_dttm
                result_time = pl.coalesce("lab_result_dttm", "lab_collect_dttm")
                lb_with_time = lb_results.filter(result_time.is_not_null())
                if len(lb_with_time) > 0:
                    frames.append(lb_with_time.select(
                        pl.col("hospitalization_id"),
                        result_time.alias("time"),
                        (pl.lit("LAB_RESULT//") + pl.col("lab_category").cast(pl.Utf8)).alias("code"),
                        pl.col("lab_value_numeric").cast(pl.Float64).alias("value"),
                        pl.lit(None).cast(pl.Utf8).alias("value_cat"),
                    ))

    # ── 6. Meds Continuous ──────────────────────────────────────────
    meds_c = loader.load_for_hospitalizations("medication_admin_continuous", all_hosp_ids)
    if len(meds_c) > 0:
    ...
    # ── 7. Meds Intermittent ────────────────────────────────────────
    meds_i = loader.load_for_hospitalizations("medication_admin_intermittent", all_hosp_ids)
    if len(meds_i) > 0:
    ...
    if len(resp) > 0:
        ...
        frames.append(_simple_events(
            resp, "recorded_dttm",
            pl.lit("RESP//") + pl.col("device_category").fill_null("unknown").cast(pl.Utf8),
        ))
        # Parameter events
        for param_col, param_name in [
            ("fio2_set", "fio2"), ("peep_set", "peep"),
    ...
    if len(assess) > 0:
        ...
        if len(a) > 0:
            frames.append(_simple_events(
                ...
                val_col="numerical_value" if "numerical_value" in a.columns else None,
                val_cat_col="categorical_value" if "categorical_value" in a.columns else None,
            ))
    ...
    try:
        ...
        if "hospitalization_id" in cs.columns:
            ...
            if len(cs_filtered) > 0:
                frames.append(_simple_events(
                    cs_filtered, "start_dttm",
                    pl.lit("CODE_STATUS//") + pl.col("code_status_category").cast(pl.Utf8),
                ))
        else:
            ...
            pids = [hosp_to_patient[hid] for hid in all_hosp_ids if hid in hosp_to_patient]
            cs_filtered = cs.filter(pl.col("patient_id").is_in(pids))
            if len(cs_filtered) > 0:
                cs_joined = cs_filtered.join(
                    hosp_map_df, left_on="patient_id", right_on="_patient_id", how="inner"
                ).filter(
    ...
    if len(crrt) > 0:
        frames.append(_simple_events(
            crrt, "recorded_dttm",
            pl.lit("CRRT//") + pl.col("crrt_mode_category").fill_null("unknown").cast(pl.Utf8),
        ))
        for param_col, param_name in [
    ...
    if len(ecmo) > 0:
        frames.append(_simple_events(
            ecmo, "recorded_dttm",
            pl.lit("ECMO//") + pl.col("device_category").fill_null("ECMO").cast(pl.Utf8),
        ))
        if "flow" in ecmo.columns:
            flow_df = ecmo.filter(pl.col("flow").is_not_null())
            if len(flow_df) > 0:
                frames.append(flow_df.select(
                    pl.col("hospitalization_id"),
                    pl.col("recorded_dttm").alias("time"),
    ...
    if len(procs) > 0:
        ...
        if len(p) > 0 and "procedure_billed_dttm" in p.columns:
            p = p.filter(pl.col("procedure_billed_dttm").is_not_null())
            if len(p) > 0:
                frames.append(_simple_events(
                    p, "procedure_billed_dttm",
    ...
    if len(diags) > 0:
        ...
        disch_df = pl.DataFrame({
            "hospitalization_id": [hid for hid, h in hosp_lookup.items() if h.get("discharge_dttm") is not None],
            "_billing_time": [
        ...
        if len(billing) > 0:
            frames.append(billing.select(
                pl.col("hospitalization_id"),
                pl.col("_billing_time").alias("time"),
                (pl.lit("ICD//") + pl.col("diagnosis_code").cast(pl.Utf8)).alias("code"),
                pl.lit(None).cast(pl.Float64).alias("value"),
                pl.lit(None).cast(pl.Utf8).alias("value_cat"),
            ))
    ...
    all_events = (
        all_events
        .join(admit_df, on="hospitalization_id", how="left")
