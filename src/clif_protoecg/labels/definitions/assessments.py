...
def _get_assess(hosp_row: dict, tables: dict) -> pl.DataFrame:
    ...
    if len(assess) == 0:
        return assess
    return assess.filter(pl.col("hospitalization_id") == hosp_row["hospitalization_id"])
...
def compute_gcs_low(hosp_row: dict, tables: dict, **kw) -> dict | None:
    ...
    if len(assess) == 0:
        return None
    low = assess.filter(
        (pl.col("assessment_category") == "gcs_total") & (pl.col("numerical_value") < 8)
    ).sort("recorded_dttm")
    if len(low) > 0:
...
@label(
    name="delirium",
    token="LABEL//delirium",
    category="clif_only",
    description="CAM-ICU positive",
    source_tables=["clif_patient_assessments"],
)
def compute_delirium(hosp_row: dict, tables: dict, **kw) -> dict | None:
    ...
    if len(assess) == 0:
        return None
    cam = assess.filter(pl.col("assessment_category") == "cam_total")
    if len(cam) == 0:
        return None
